#include<bits/stdc++.h>
using namespace std;
map<string,int> funcMap;//typable check
map<string,int> hmap;//tokens
...
string PredTypable(vector<string> arg){
    if(arg[0].compare("0")==0){
        cout<<"T-Pred Applied"<<endl;
        return "0";
    }
        ...
        return PredTypable(checkTypable(arg));
    }
}
...
string IsZeroTypable(vector<string> arg){
    i-f(arg[0].compare("0")==0){
        cout<<"T-isZero Applied"<<endl;
        return "Bool";
    }
    else if(arg[0].compare("True")==0 || arg[0].compare("False")==0 || arg[0].compare("Bool")==0){
        cout<<"Ill-Typed Term"<<endl;
